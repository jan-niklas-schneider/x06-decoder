...
function lookupSequence(seq){return DB?.stations?.find(s=>s.sequence===seq)??null;}
...
function tick(){
  ...
  if(!autolock){
    ...
    const alreadyCommitted = seqDigits.length && seqDigits[seqDigits.length-1]===det.digit && (now-lastCommitTs)<(TONE_DURATION_MS*0.6);
...
function applyDbFilter(){
  ...
  dbFiltered = !q ? all : all.filter(s => s.sequence.includes(q) || (s.target||"").toLowerCase().includes(q));
